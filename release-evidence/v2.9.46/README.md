...
[![tests: 1307 passing](https://img.shields.io/badge/tests-1307%20passing-brightgreen.svg)](cpm-engine.test.js)
[![crossval: JS↔Python 1167/1183](https://img.shields.io/badge/JS%E2%86%94Python-1167%2F1183-brightgreen.svg)](DAUBERT.md#31-independent-verification)
[![coverage: 93%](https://img.shields.io/badge/coverage-93%25%20stmts%20%2F%2082%25%20branches-brightgreen.svg)](DAUBERT.md#21-test-coverage-v2946-baseline)
...
**Open source. AACE-aligned. Daubert-disclosed. 1,307 JS unit tests, plus JS/Python parity on 1167 of 1183 enumerated cross-validation comparisons; the remaining 16 are not compared because neither engine emits the field on a completed activity (see [DAUBERT.md §3.1](DAUBERT.md#31-independent-verification)).**
...
console.log('Engine version:', result.manifest.engine_version); // 2.9.46
```
...
> Fitkowski, D. (2026). *cpm-engine: An open-source critical-path-method engine with AACE-canonical method labels and a published Daubert disclosure.* Critical Path Partners. Version 2.9.46. <https://github.com/danafitkowski/cpp-cpm-engine>
...
See [CHANGELOG.md](CHANGELOG.md) for the full release history through v2.9.46.

---

## Contributing

See [`CONTRIBUTING.md`](CONTRIBUTING.md). Forensic correctness is enforced — every commit must pass 1,307 unit tests and the cross-validation harness, in which 1167 of 1183 defined comparisons execute and are bit-identical with 0 failures across 53 fixtures (the remaining 16 are skipped rather than compared — 8 `ff_signed`, 8 `ff_signed_working_days`, all on completed activities where neither engine emits the field), plus the citation regression, truncation regression, and version-drift regression gates (all wired into `npm run test:all` and `npm run verify`). New citations require WebSearch-verified URLs. No fabricated case names. No LLM-generated narratives in core engine paths.
