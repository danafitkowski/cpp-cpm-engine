...
**Rule.** The JavaScript engine and the Python reference implementation must produce bit-identical output on every field the cross-validation harness compares, on every fixture. Read the scope literally: the harness compares a field only when both sides emit it, and for the free-float fields only when neither side is null, so a field one engine emits and the other does not is skipped rather than failed and is counted in neither half of the printed checks ratio. A skipped comparison can never move that ratio, so the figure reads executed-over-executed. As measured 2026-08-27, 6 comparisons are skipped across 3 of the 46 fixtures, leaving 1009 of a 1015-comparison surface actually executed. All 6 are null-versus-undefined artifacts on completed activities where neither engine emits the field, so none is an open port gap. The 58 skips that were open port gaps — activities where the JavaScript engine emitted `ff_signed_working_days` and the Python reference never assigned it — closed when the has-successors branch was ported. As measured 2026-08-27: **1009 of 1015 enumerated comparisons executed and bit-identical** across 46 fixtures. The other 6 are skipped by the harness field guards rather than compared, all on completed activities where neither engine emits the field. Note that `npm run crossval` prints `1009 / 1009`; that denominator is the executed count, not the enumerated surface. The run also prints a `Skipped:` line and a `Surface:` line, so the shortfall is no longer invisible to the harness or the exit code. Plus a 282-activity real-XER stress test with 0 mismatches, on a single non-public XER that is kept locally, not committed, and not independently reproducible from this repo. Backed by 1,273 JavaScript unit tests (as of v2.9.43).
