...
**Rule.** The JavaScript engine and the Python reference implementation must produce bit-identical output on every field the cross-validation harness compares, on every fixture. Read the scope literally: the harness compares a field only when both sides emit it, and for the free-float fields only when neither side is null, so a field one engine emits and the other does not is skipped rather than failed and is counted in neither half of the printed checks ratio. A skipped comparison can never move that ratio, so the figure reads executed-over-executed. As of v2.9.39, 64 comparisons are skipped across 34 of the 45 fixtures, leaving 925 of a 989-comparison surface actually executed. 58 of the 64 are activities where the JavaScript engine emits `ff_signed_working_days` and the Python reference never assigns it; the other 6 are null-versus-undefined artifacts on activities where neither engine emits the field. Those 64 are open port gaps in the Python reference, not agreements, and they are not yet surfaced by the harness, the exit code, or the attestation. As of v2.9.39: **925 of 989 enumerated comparisons executed and bit-identical** across 45 fixtures. The other 64 are skipped by the harness field guards rather than compared: 58 because the Python reference never assigns `ff_signed_working_days` on the has-successors path, and 6 on completed activities where neither engine emits the field. Note that `npm run crossval` prints `925 / 925`; that denominator is the executed count, not the enumerated surface. Plus a 282-activity real-XER stress test with 0 mismatches, on a single non-public XER that is kept locally, not committed, and not independently reproducible from this repo. Backed by 1,134 JavaScript unit tests.
