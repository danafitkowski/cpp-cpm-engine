...
[![crossval: JS↔Python 931/995](https://img.shields.io/badge/JS%E2%86%94Python-931%2F995-brightgreen.svg)](DAUBERT.md#31-independent-verification)
...
**Open source. AACE-aligned. Daubert-disclosed. 1,134 JS unit tests, plus JS/Python parity on 931 of 995 enumerated cross-validation comparisons; the remaining 64 are not compared because the Python reference does not emit the field (see [DAUBERT.md §3.1](DAUBERT.md#31-independent-verification)).**
...
console.log('Engine version:', result.manifest.engine_version); // 2.9.40
```
...
```bash
npm run crossval
# 45 fixtures, 931 checks executed, 0 deviations as of v2.9.41.
# A further 64 comparisons on the enumerated surface (995 total) are skipped rather than failed by the harness field guards, all on ff_signed / ff_signed_working_days, which the Python reference does not always emit.
```
...
> Fitkowski, D. (2026). *cpm-engine: An open-source critical-path-method engine with AACE-canonical method labels and a published Daubert disclosure.* Critical Path Partners. Version 2.9.40. <https://github.com/danafitkowski/cpp-cpm-engine>
