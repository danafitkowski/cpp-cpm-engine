...
**Engine version:** `cpm-engine` v2.9.33
**Generated:** 2026-05-23
**Status:** Engine outputs captured for all 15 cases; **P6-native values pending analyst capture**.

This document is the per-case verdict roll-up across the 15-case P6 comparison matrix. Once the analyst populates the `*_p6` columns of each case's `comparison.csv` and marks the `verdict_pass_fail` column, this file gets updated with the per-case verdict (PASS / FAIL / PARTIAL).
...
| # | Case ID | Engine Project Finish (v2.9.33) | Engine Alert Count | P6 Capture Verdict | Notes |
|---|---|---|---|---|---|
...
| 14 | `14-fractional-lag` | 2026-01-14 | 10 | ⏳ pending | SUB_DAY_LAG_ROUNDED ALERT — engine rounds, P6 honors hours |
| 15 | `15-dangling-relationship` | 2026-01-13 | 9 | ⏳ pending | dangling-rel ALERT — engine drops, P6 cannot create the input |
...
| Aggregate | Value |
|---|---|
| Cases capturable in P6 (cases 1–13) | 13 |
| Cases not capturable in P6 (case 14 sub-day lag known divergence; case 15 dangling-rel cannot be authored in P6) | 2 |
...
These two cases test the engine's handling of inputs P6 itself cannot produce. They will not have a P6 verdict by design:

### Case 14 — Fractional lag
...
The remaining 13 cases ARE capturable in P6 and should be field-identical.
...
P6 comparison matrix for cpm-engine v2.9.33:
  ...
  Reproduction:  https://github.com/danafitkowski/cpp-cpm-engine/tree/v2.9.33/validation/p6-comparison
```
