...
The directive is quoted as written, and its 747 was the crossval figure of the day: 747 checks across 43 fixtures, which held from v2.9.27 through v2.9.38. As of v2.9.39 the harness executes 925 checks across 45 fixtures and prints `925 / 925`. That ratio counts comparisons executed, not the comparison surface, because each optional-field guard skips a comparison instead of failing it when either side carries no value. 64 comparisons are skipped that way (61 on `ff_signed_working_days`, 3 on `ff_signed`), every one of them a field where the Python reference emits null, so internal parity today is 925 of a 989-comparison surface. The gap is an unimplemented field in the Python port (`python_reference/cpm.py` assigns `ff_signed_working_days` only on the no-successors branch), not a disagreement on dates, total float, or published free float: those, plus ES/EF/LS/LF and `tf_working_days`, are compared on all 105 activity groups. The directive's point is unchanged. Internal parity is not P6 equivalence.
