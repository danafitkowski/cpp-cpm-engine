...
- Verify the deliverable's report manifest references:
  - Engine version (e.g., `cpm-engine v2.9.45`)
...
1. **Attacks on principles.** "The engine itself is unreliable, the math is wrong, the validation is insufficient." → The engine's [DAUBERT.md](DAUBERT.md) + [VERIFY_RELEASE.md](VERIFY_RELEASE.md) + the v2.9.45 verification chain answer this layer.
...
This SOP is aligned to `cpm-engine` v2.9.45. SOP revisions are tracked in [`CHANGELOG.md`](CHANGELOG.md) under the engine version that introduced them.
