# Daubert / FRE 702 Disclosure — `cpm-engine` v2.9.47
...
The engine's correctness is exercised on six surfaces. Five are tabulated below; the sixth is the engine-vs-P6 comparison matrix described in §8.5 and shipped at [`validation/p6-comparison/`](validation/p6-comparison/), a 15-case matrix in which 13 cases are scored against a single Primavera P6 23.12 capture and read 13 / 13 PASS, the remaining 2 being by-construction non-comparable (one a deliberate engine divergence: day-granular rounding of sub-day lags, which P6 stores in hours and honors natively; one an input P6 cannot author: a relationship to a non-existent activity), excluded and documented in `validation/engine-limitations/`. That matrix is disclosed as fitted rather than held out: the first capture scored 6 of 13, five divergence families were then corrected in the engine against P6's pinned answers, and the matrix was regenerated, so no post-fix independent capture exists and the capture sheet itself is not committed to this repository. The five tabulated surfaces are not mutually independent either: branch and statement coverage is `c8` instrumentation over the unit suite at the 2026-08-27 coverage baseline (then 1,216 tests) rather than a separate test of the math, and the cross-validation, coverage and public-API rows all exercise code authored by the proponent. §3.1 addresses what proponent authorship does and does not close.

| Surface                    | Coverage                                                                                          | Result          |
|----------------------------|---------------------------------------------------------------------------------------------------|-----------------|
| Unit tests                 | `cpm-engine.test.js` — date helpers, calendar arithmetic, topo sort, Tarjan SCC, forward/backward pass, salvage mode, all strategy modes (TFM/LPM/MFP with divergence), kinematic delay dynamics, topology hash, Daubert disclosure, Bayesian update, multi-jurisdiction default holiday rule sets (66 jurisdiction codes, framework-aligned per [`docs/jurisdictions.md`](docs/jurisdictions.md); analysts must verify currency against the operative statute for forensic use), P6 primary + secondary constraints, TT_Hammock two-pass with full SS/FF/SF semantics, FF/SF relationship coverage, ALAP backward-pass tightening, Section D MC-constraint enforcement, hammock visited-set memoization, dateToNum 2-digit guard, Round 6 strong-assertion strengthening, Round 7 full hammock SS/FF/SF semantics, Round 8 R8A engine math fix wave, Round 9 v2.9.12 engine math fix wave, v2.9.22 audit HIGH wave (10 items: rel-trim, strict parse, typed input, getHolidays year-clamp, etc.), v2.9.23 small-batch wave (16 items: dedup, codepoint cmp, target_drtn WARN, etc.), v2.9.26 provenance + citation polish, v2.9.27 paired JS+Python fixes (R6 completed-succ skip, R12 data_date snap, R21 Python MonFri fast path, R9 tf_working_days backport, F24 ff/ff_working_days backport, A12 hash hardenings, R10 project_calendar fallback, R6 MS_Start widen WARN), CI-1..CI-18 finish-constraint-instant checks (v2.9.45), RL-5/RL-6/RL-7 re-pinned to SS_U's P6-measured values (v2.9.46), FA-1..FA-8 completed-predecessor unexpired-lag checks and RL-5/RL-6/RL-7 re-described for the unexpired lag off started work (v2.9.47). | **1,315 / 1,315 passing** |
| Cross-validation suite     | `cpm-engine.crossval.js` — 82 fixtures, 1957 of 2011 checks executed, JS engine vs Python `compute_cpm` reference. The 54 unexecuted checks are confined to `ff_signed` (27) and `ff_signed_working_days` (27) on completed activities, where NEITHER engine emits the field; the harness skips those rather than failing them, so its `1957 / 1957` tally counts executed checks only. ES/EF/LS/LF, TF, `tf_working_days`, `ff` and `ff_working_days` were each compared on all activity groups with no skips. Parity asserted on the enumerated comparison surface: forward/backward pass dates (ES/EF/LS/LF), Kahn topo order, critical-path codes, project finish, TF, TF working days, FF, FF working days, and signed FF, over fixtures that exercise FS/SS/FF/SF relationship types and lags; alert counts and severity are compared on 78 of the 82 fixtures. Three exclusions are disclosed rather than hidden. (1) Tarjan SCC is not part of this surface: `_tarjan_scc` was stripped from `python_reference/cpm.py` for OSS distribution (stated in that file's own header), and the Python reference detects cycles through `_topo_sort`'s `has_cycle` flag raising `ValueError`, so the two cycle fixtures (F23, F31) assert only that both engines refused and never compare an SCC decomposition. Tarjan SCC is covered by the JS unit suite instead (`cpm-engine.test.js` Section B plus the 5,000-node iterative-Tarjan overflow test), which is where the unit-test row above already claims it. (2) Alert parity is not compared on F23 and F31, where both engines throw and there is no output to compare. Nor is it compared on F65 and F67, where the JS engine also emits its per-activity future-actual-finish ALERT. The Python reference has never carried that ALERT: this is a pre-existing gap that the future-actual fixtures surface, and it is disclosed here rather than hidden. A former three-fixture carve-out (F20, F21, F27, where the out-of-sequence ALERT was believed JS-only) was retired on 2026-08-19: the Python reference has emitted that alert since the v2.9.27 paired-fix wave, and those six alert comparisons now execute and pass. (3) Neither engine emits `ff_signed` or `ff_signed_working_days` for completed activities, so 54 of the 2011 enumerated comparisons are never executed (27 on `ff_signed` and 27 on `ff_signed_working_days`). They sit on these completed activities: F10, F20 and F43, one pair each; added at v2.9.46, F51, F52 and F53, one pair each, and F54, which carries two completed activities on its pass-through chain and so contributes two pairs; added at v2.9.47, F61, F65-F74 and F82-F84, one pair each, F75, two pairs, and F85, three pairs. In every one of the 54 both engines are silent, so none is a one-sided gap. The former one-sided gap — 58 comparisons where JS emitted a value and `python_reference/cpm.py` emitted none, because the reference did not assign `ff_signed_working_days` on the has-successors path — closed when that branch was ported. The harness therefore executes, and passes, 1957 of the 2011 enumerated comparisons. v2.9.27 expanded crossval coverage by backporting tf_working_days, ff, and ff_working_days fields to Python (previously documented JS-only gaps in F24). v2.9.46 added 7 fixtures (F51-F57) exercising retained-logic pass-through, SS_U and PO_SNAP directly. v2.9.47 added 29 fixtures (F58-F86) exercising the unexpired lag off started and completed work, the "lag from Actual Start" option and SS/SF links into completed work directly. Bayesian and kinematic surfaces are JS-only and **explicitly excluded** from the bit-identical claim — see §8. | **1957 of 2011 enumerated comparisons executed and bit-identical; 54 skipped rather than failed (all 54 on completed activities where neither engine emits a signed-FF value)** |
...
| Branch + statement coverage | `c8` instrumentation over `cpm-engine.js` exercised by the unit suite (1,315 tests at the 2026-09-23 baseline capture). Reported as statement / branch / function / line coverage; see §2.1 for the coverage baseline, its measurement provenance, and the disclosed uncovered-line cluster. | **93.95% stmts / 83.08% branches / 95.20% funcs / 93.95% lines** |
...
### §2.1 Test Coverage (v2.9.47 baseline)

Coverage is measured via [`c8`](https://github.com/bcoe/c8) over `cpm-engine.js` exercised by the unit suite, captured via `npm run coverage`; the baseline below was captured 2026-09-23 over the 1,315-test suite, on the v2.9.47 bytes. Reported numbers are not editorial — they are emitted by the test runner.

| Coverage Surface | Count | Pct |
|---|---|---|
| Statements | 10,415 / 11,085 | **93.95%** |
| Branches | 2,249 / 2,707 | **83.08%** |
| Functions | 139 / 146 | **95.20%** |
| Lines | 10,415 / 11,085 | **93.95%** |

**Disclosed uncovered clusters.** A defensible coverage disclosure must name what is not covered, not just what is. The regions below are named in prose; the line-level list is generated rather than editorial. `npm run coverage` writes `coverage/cpm-engine.js.html`, which marks every uncovered statement in the bytes you are reading: 649 uncovered statements. Cite that HTML report rather than the console summary, whose `Uncovered Line #s` column is width-truncated. The uncovered statement clusters fall in these regions:
...
The output should match the coverage baseline in §2.1 within rounding: 93.95% statements (10,415 / 11,085), 83.08% branches (2,249 / 2,707), 95.20% functions (139 / 146), 93.95% lines, measured on the v2.9.47 bytes (2026-09-23). The v2.9.47 unexpired-lag wave moved engine math, and the baseline WAS re-measured on those bytes for this release; the prior baseline (93.81 / 82.63 / 94.96, measured on the v2.9.46 bytes 2026-09-22) is superseded. Drift beyond rounding on the v2.9.47 tag documents itself.
...
gh attestation verify release-evidence/v2.9.47/witness-v2.9.47.json --owner danafitkowski

# Equivalent public route — note the release asset is named `latest.json`,
# not `attestations-latest.json`:
#   gh release download v2.9.47 --repo danafitkowski/cpp-cpm-engine --pattern 'latest.json'
...
**Cross-validation reports 1957 executed comparisons with 0 deviations across 82 fixtures on the enumerated CPM comparison surface (forward/backward pass dates, Kahn topo order, critical-path codes, project finish, FF/SF working-day arithmetic, TF, TF working days, FF, FF working days, and signed FF where both engines emit it; alert counts and severity are compared on 78 of the 82 fixtures. They are skipped on the 2 cycle fixtures, which compare refusal only (the former three-fixture out-of-sequence carve-out was retired 2026-08-19, the Python reference having emitted that alert since v2.9.27), and on F65 and F67, where the JS engine's per-activity future-actual-finish alert has no Python counterpart (a pre-existing, disclosed gap); Tarjan SCC is JS-unit-tested only and is not on this surface, having been stripped from `python_reference/cpm.py`). The harness prints `1957 / 1957`, but that denominator is the executed count, not the comparison surface: the full enumerated surface is 2011 comparisons, and 54 of them are skipped by the harness field guards (27 on `ff_signed`, 27 on `ff_signed_working_days`) and are neither counted nor failed. None of those 54 is substantive: all are null-vs-undefined artifacts on completed activities where neither engine emits the field. The 58 substantive skips disclosed through v2.9.41 closed when the Python reference began assigning `ff_signed_working_days` on the has-successors path. So 0% observed deviation holds over the 1957 executed comparisons, and agreement over the full 2011-comparison surface is 1957 of 2011 by the harness's own equality semantics. Bayesian and kinematic surfaces are JS-only and excluded (see §8).**
**Real-XER stress reports 282 / 282 = 0% deviation (single non-public reference XER; not committed, not independently reproducible).**

This is the engine's **observed** error rate on the disclosed validation suite as measured 2026-09-23 (82 fixtures / 1957 cross-validation checks). It is not a general error-rate claim; it is the rate at which the engine has matched its Python sibling reference and a 282-activity P6 reference under the test surface defined in §2.

Performance characteristics:

- 1,315 unit tests run on Node 18.
- 5,000-node linear chain Tarjan SCC in **~8 ms**.
...
**No known silent wrong-answer paths remain on the disclosed validation surface after v2.1.0.** Every degenerate input either throws a labeled exception or appears in the salvage log. The 1,315-test unit suite, the 82-fixture / 1957-check JS↔Python crossval, and the 83.08% branch-coverage instrumentation (v2.9.47 baseline, §2.1) are the surface on which this claim is observed; paths outside that surface are not warranted.
...
```js
result.manifest = {
    engine_version: '2.9.47',                   // Synchronized with package.json (bump per release)
...
2. Run `npm run test:all` to reproduce the full 11-gate chain: the 1,315 unit tests and the 1957-of-2011 cross-validation checks across 82 fixtures (3,272 counted checks), plus the citation, client-name, truncation, version-drift, SOP, reissue, crypto-signoff, P6-comparison and corpus-DAG regression gates. Or `npm run verify` for the attestation-witness flow, which invokes five of those gates and records each in the witness JSON.
...
**Disclosure.** Opposing experts can audit every constraint applied during a run by filtering `result.alerts` on the contexts above. Pair with `result.manifest.engine_version === '2.9.47'` to confirm the constraint module version.
...
Those 33 strict-mode tests are still part of the engine's unit-test suite in the current v2.9.47 release (1,315 total tests including strict-mode coverage).
