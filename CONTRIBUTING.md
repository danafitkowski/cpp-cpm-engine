...
1. **Every commit must pass 1,307 unit tests, 0 fail.**

   ```bash
   npm test
   ```

2. **Every commit must pass the cross-validation harness against the Python sibling: 1167 of 1183 defined comparisons executed and bit-identical, 0 failures, across 53 fixtures; 16 comparisons are skipped rather than compared (8 `ff_signed`, 8 `ff_signed_working_days`, all on completed activities where neither engine emits the field).**
...
- [ ] `npm test` passes (1,307 tests, 0 fail).
- [ ] `npm run crossval` passes (1167 of 1183 defined comparisons executed and bit-identical, 0 failures, across 53 fixtures; 16 skipped rather than compared).
