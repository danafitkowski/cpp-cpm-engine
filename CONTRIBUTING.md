...
1. **Every commit must pass 1,315 unit tests, 0 fail.**

   ```bash
   npm test
   ```

2. **Every commit must pass the cross-validation harness against the Python sibling: 1957 of 2011 defined comparisons executed and bit-identical, 0 failures, across 82 fixtures; 54 comparisons are skipped rather than compared (27 `ff_signed`, 27 `ff_signed_working_days`, all on completed activities where neither engine emits the field).**
...
- [ ] `npm test` passes (1,315 tests, 0 fail).
- [ ] `npm run crossval` passes (1957 of 2011 defined comparisons executed and bit-identical, 0 failures, across 82 fixtures; 54 skipped rather than compared).
