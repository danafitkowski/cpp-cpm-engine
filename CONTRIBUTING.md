...
1. **Every commit must pass 1,134 unit tests, 0 fail.**

   ```bash
   npm test
   ```

2. **Every commit must pass 925/925 cross-validation checks against the Python sibling (45 fixtures).**
...
- [ ] `npm run crossval` passes (925/925 checks across 45 fixtures).
