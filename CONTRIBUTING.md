...
1. **Every commit must pass 1,213 unit tests, 0 fail.**

   ```bash
   npm test
   ```

2. **Every commit must pass the cross-validation harness against the Python sibling: 1009 of 1015 defined comparisons executed and bit-identical, 0 failures, across 46 fixtures; 6 comparisons are skipped rather than compared (3 `ff_signed`, 3 `ff_signed_working_days`, all on completed activities where neither engine emits the field).**
...
- [ ] `npm test` passes (1,213 tests, 0 fail).
- [ ] `npm run crossval` passes (1009 of 1015 defined comparisons executed and bit-identical, 0 failures, across 46 fixtures; 6 skipped rather than compared).
